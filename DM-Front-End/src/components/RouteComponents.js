...
export const AdminRoute = ({currentUser, dispatch, component, ...rest}) => {
  if (!JSON.parse(localStorage.getItem('user-data')).dm_user_type == 'omni' || !Login.isAuthenticated(localStorage.getItem('token'))) {
    return (<Redirect to="/app/main"/>)
  } else if (JSON.parse(localStorage.getItem('user-data')).dm_user_type === 'omni') {
    return (
